...
export default function TheatersComponent(){
    ...
    var response = axios.get(`https://ticket-booking-app-nodejs.herokuapp.com/user/theater/${urlParams.theatername}`,{
        headers:{
            'x-access-token':s
        }
    ...
    return(
        ...
        <div className="moviename">{urlParams.theatername}</div>
        ...
        <div className="theater-for-movies">
            ...
            <div className="showtiming">
                {ele.showlist.map(elem =><button className="timing-buttons" onClick={(e) => navigate(`/${urlParams.theatername}/${elem}/${dateofBooking}`)}>{dict[elem]}</button>)}
            </div>
        </div>
        </>)}
        </div>
